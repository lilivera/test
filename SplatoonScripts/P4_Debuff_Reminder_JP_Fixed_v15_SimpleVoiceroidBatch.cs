...
{
    public class P4_Debuff_Reminder_JP_Fixed_v15_SimpleVoiceroidBatch : SplatoonScript
    {
        public override Metadata Metadata { get; } = new Metadata(15, "NightmareXIV / JP fixed / large white text / SimpleVoiceroid2Proxy batched");
        public override HashSet<uint> ValidTerritories { get; } = new HashSet<uint> { 1363 };

        private const float TtsTriggerSeconds = 7.0f;
        private const string VoiceProxyUrl = "http://localhost:4532/talk";
        private const int VoiceProxyTimeoutMs = 3000;
        private const int VoiceProxyBatchWindowMs = 120;

        private readonly object _voiceQueueLock = new object();
        private readonly Queue<string> _voiceQueue = new Queue<string>();
        private bool _voiceWorkerRunning = false;
        ...
        private void Speak(string text)
        {
            ...
            try
            {
                lock (_voiceQueueLock)
                {
                    _voiceQueue.Enqueue(text);

                    if (_voiceWorkerRunning)
                    {
                        return;
                    }

                    _voiceWorkerRunning = true;
                }

                ThreadPool.QueueUserWorkItem(_ => VoiceQueueWorker());
            }
            catch
            {
            ...
            try
            {
                while (true)
                {
                    ...
                    lock (_voiceQueueLock)
                    {
                        ...
                        {
                            _voiceWorkerRunning = false;
                            return;
                        }

                        firstText = _voiceQueue.Dequeue();
                        batch.Add(firstText);
                    }

                    if (!IsVoiceCommand(firstText))
                    {
                        Thread.Sleep(VoiceProxyBatchWindowMs);

                        lock (_voiceQueueLock)
                        {
                            while (_voiceQueue.Count > 0 && !IsVoiceCommand(_voiceQueue.Peek()))
                            {
                                batch.Add(_voiceQueue.Dequeue());
                            }
                        }
                    }

                    var text = string.Join("、", batch.Where(x => !string.IsNullOrWhiteSpace(x)).ToArray());
                    if (!string.IsNullOrWhiteSpace(text))
                    {
                        PostTalk(text);
                    }
                    ...
                    {
                        Thread.Sleep(EstimatePostGapMs(text));
                    }
                }
            }
            catch
        ...
        private int EstimatePostGapMs(string text)
        {
            if (string.IsNullOrWhiteSpace(text))
            {
                return 1000;
        ...
        private void ClearLocalVoiceQueue()
        {
            lock (_voiceQueueLock)
            {
                _voiceQueue.Clear();
        ...
        private void PostTalk(string text)
        {
            try
            {
                ...
                var request = (HttpWebRequest)WebRequest.Create(VoiceProxyUrl);
                request.Method = "POST";
                request.ContentType = "application/json; charset=utf-8";
                request.ContentLength = data.Length;
                request.Timeout = VoiceProxyTimeoutMs;
                request.ReadWriteTimeout = VoiceProxyTimeoutMs;
                request.Proxy = null;
                ...
                {
                    // Nothing to do. Success means SimpleVoiceroid2Proxy accepted the text.
                }
            }
            catch
            {
                // Ignore HTTP/TTS errors so visual hints continue to work.
            }
        }
        ...
        {
            if (ImGui.Button("VOICEROIDテスト"))
            {
                Speak("テスト、散開");
            }

            if (ImGui.Button("VOICEROIDテスト 複数文"))
            {
                Speak("見ない、散開、止まる");
            }

            if (ImGui.Button("VOICEROIDテスト 3件連続"))
            {
                Speak("テスト1、見る");
                Speak("テスト2、散開");
                Speak("テスト3、止まる");
            ...
            if (ImGui.Button("VOICEROID 割り込みOFF"))
            {
                Speak("<interrupt_disable>");
            }

            if (ImGui.Button("VOICEROID 割り込みON"))
            {
                Speak("<interrupt_enable>");
            }

            if (ImGui.Button("VOICEROID キュークリア"))
            {
                ClearLocalVoiceQueue();
                Speak("<clear>");
            }
        }
